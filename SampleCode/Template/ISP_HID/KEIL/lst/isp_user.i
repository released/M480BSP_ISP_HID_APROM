...
__attribute__((aligned(4))) static uint8_t aprom_buf[0x1000UL];
...
uint8_t rtc_read_magic_tag(void)
{
    ...
    ((RTC_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x01000UL))->RWEN = 0x0000A965UL;
    while(!(((RTC_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x01000UL))->RWEN & (0x1ul << (16))));
    
    tag =  (((RTC_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x01000UL))->SPR[(0)]);

    printf("\033[1;36m" "[LDROM]" "Read MagicTag <0x%02X>\r\n" "\033[0m",tag);
...
void rtc_write_magic_tag(uint8_t tag)
{
    ...
    (((RTC_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x01000UL))->SPR[(0)] = (tag));
    
    printf("\033[1;36m" "[LDROM]" "Write MagicTag <0x%02X>\r\n" "\033[0m",tag);
}
...
static uint16_t CalCheckSum(uint32_t start, uint32_t len)
{
    ...
    for (i = 0; i < len; i += 0x1000UL)
    {
        ReadData(start + i, start + i + 0x1000UL, (uint32_t *)aprom_buf);

        if (len - i >= 0x1000UL)
        {
            lcksum += Checksum(aprom_buf, 0x1000UL);
        }
        else
        {
...
int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    *((volatile unsigned int *)(response + 4)) = (0);
    pSrc += 8;
    srclen -= 8;
    ReadData(0x00300000UL, 0x00300000UL + 16, (uint32_t *)(response + 8)); 
    ...
    {
        *((volatile unsigned int *)(response + 8)) = (((SYS_T *) (((uint32_t)0x40000000) + 0x00000UL))->PDID);
        goto out;
    }
    else if (lcmd == 0x000000AB || lcmd == 0x000000AC || lcmd == 0x000000AD)
    {
        ...
        printf("\033[1;36m" "[LDROM]" "Perform CHIP_RST...\r\n" "\033[0m");
...
#line 170 "..\\isp_user.c"
    ...
    {
        EraseAP(0x00000000UL, (g_apromSize < g_dataFlashAddr) ? g_apromSize : g_dataFlashAddr); 
    ...
    else if (lcmd == 0x000000CA)
    {
        ...
        *((volatile unsigned int *)(response + 8)) = ((((FMC_T *) (((uint32_t)0x40000000) + 0x0C000UL))->ISPCTL & 0x2) ? 2 : 1);
    }
    ...
    else if (lcmd == 0x000000FF)     
    {
        ...
        PageAddress = StartAddress & (0x100000 - 0x1000UL);
        ...
        if ((StartAddress % 0x1000UL) >= (0x1000UL - LastDataLen))
        {
            FMC_Erase_User(PageAddress + 0x1000UL);
        }
